...
export async function fornecedorRoutes(app: FastifyInstance) {
  ...
  // M5-05 — Ranking relativo de fornecedores por categoria
  app.get('/fornecedores/ranking', async (request, reply) => {
    ...
    // Busca todos os fornecedores ativos da organização
    const fornecedores = await prisma.fornecedor.findMany({
      where: { idOrganizacao, status: 'Ativo' },
      include: {
        qualificacoes: {
    ...
    const ranking = fornecedores.map(f => {
      ...
      const entregasConfirmadas = todasEntregas.filter(e => e.status === 'Confirmada').length
      const entregasContestadas = todasEntregas.filter(e => e.status === 'Contestada').length
    ...
    const rankingOrdenado = ranking
      ...
      .map((item: typeof ranking[0], index: number) => ({ posicao: index + 1, ...item }))
